...
INSERT INTO Person VALUES(id, email, password, first_name, surname);
INSERT INTO Phone VALUES(id, phone_no);
-- if id is for student
INSERT INTO Student VALUES(id, Null, Null, CURRENT_TIMESTAMP);
-- if id is for instructor
INSERT INTO Instructor VALUES(id, Salary, CURRENT_TIMESTAMP);

-- login
SELECT id
FROM Person
WHERE id = Person.id  AND password = Person.password;

--------------------- STUDENT SQL QUERIES ---------------------------------------------------------------
-- list all courses that a student can register for
SELECT DISTINCT course_name
FROM Course, Prereq
WHERE (Course.id = Prereq.course_id
    AND Prereq.prereq_id IN (SELECT course_id FROM Courses_Taken)
    AND Course.id NOT IN (SELECT course_id FROM Courses_Taken))
    OR Course.id  NOT IN (SELECT course_id FROM Prereq) AND Course.id NOT IN (SELECT course_id FROM Courses_Taken);

-- select and register a course for a student
-- the student picks an available section from a course they can take.
INSERT INTO Courses_Registered VALUES(student_id, course_id, section_id);

-- list all registered courses of a student
SELECT course_name
FROM Course, Courses_Registered
WHERE Course.id = Courses_Registered.course_id;

-- select a course and display grades for that course
Select grade
From Take_Exam, Course_Exam
WHERE Take_Exam.exam_id IN (SELECT exam_id FROM Course_Exam)
AND Course_Exam.course_id = _course_id;

Select grade
From Take_Assignment, Course_Assignment
WHERE Take_Assignment.assignment_id IN (SELECT assignment_id FROM Course_Assignment)
...
INSERT INTO Joins_Research Values(_group_id, _student_id);

-- create study group
INSERT INTO Study_Group Values(_group_id, _student_id, _group_name, _purpose);
...
FROM Student, Participates, Study_Group
WHERE Student.id = _student_id
...
INSERT INTO Reviews VALUES(_instructor_id, _student_id, _points, _comments);

--------------------- Instructor SQL QUERIES ------------------------------------------------------------
-- list all the courses of the instructor
SELECT DISTINCT course_name
FROM Course, Section, Teaches
WHERE Teaches.instructor_id = _instructor_id
...
UPDATE Student_Sec
SET grade = _grade
WHERE section_id = _section_id AND student_id = _student_id;

-- list TA's of a course
SELECT DISTINCT name
FROM Course, TA, Assists
WHERE Assists.course_id = _course_id
    AND Assists.ta_id = TA.ta_id; 

-- authorize TA's for doing specific tasks such as submitting hw grades, attendance etc.
INSERT INTO Auth_TA VALUES(_task_desc, _course_id, FALSE);

-- list possible tasks for a TA for a specific course
SELECT task_desc
FROM Task_List ,Teaches
WHERE Teaches.instructor_id = _instructor_id 
    AND Teaches.course_id = _course_id
    AND Task_List.course_id = _course_id;

-- create an exam
INSERT INTO Exam VALUES(_exam_id, _exam_name, _start_time, _end_time);

-- create an assignment
INSERT INTO Assignment VALUES(_assignment_id, _assignment_name, _assignment_due_date);

-- create a research group
-- grant is pre-determined
INSERT INTO Research_Group VALUES(_group_id, _research_topic, _grant);
